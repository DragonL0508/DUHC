...
function duhc:api/ui/create_button {id:"items", slot:19, title:"§e如何設定起始物資?", desc:["§7大廳中心有一個初始物資箱，","§7使用§6創造模式§7將物資放入即可。"], icon:"chest", click:"", type:"normal"}

data modify storage duhc:main data.temp.range set value {min:50, max:1000, n:5}
function duhc:api/ui/create_button {id:"max_item_count", slot:20, title:"§e掉落物最大數量:", desc:["§7控制世界掉落物數量，","§7避免多人遊玩時TPS嚴重降低。"], icon:"feather", click:"function duhc:player/lobby/ui/admin/click/value", type:"value"}
